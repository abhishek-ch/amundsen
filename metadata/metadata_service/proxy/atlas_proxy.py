...
class AtlasProxy(BaseProxy):
    ...
    def _serialize_badges(self, entity) -> List[Badge]:
        """
    ...
    def _serialize_tags(self, entity) -> List[Tag]:
        """
            ...
            for chunk in resource_guids_chunks:
                ...
                if resource_type == ResourceType.Table.name:
                    ...
                    for entity in entities.entities:
                        summary, _ = self._get_dashboard(entity)
    ...
    def _get_resource_by_user_relation(self, user_email: str, relation_type: UserResourceRel,
        ...
        if resource_type.name == ResourceType.Table.name:
            root_key = ResourceType.Table.name.lower()
        elif resource_type.name == ResourceType.Table.name:
            root_key = ResourceType.Dashboard.name.lower()
        else:
    ...
    def _serialize_dashboard_queries(self, query_entities: List[AtlasEntityWithExtInfo]) -> List[DashboardQuery]:
        result = []

        for query in query_entities:
            attrs = query[CommonKeys.attrs]
    ...
    def _get_dashboard_queries(self, chart_guids: List[str]) -> List[DashboardQuery]:
        """
        Return list of Queries.

        :param charts_guids: list of chart guids
        :return : List of Amundsen DashbaardQuery objects.
        """
        dashboard_charts = self.client.entity.get_entities_by_guids(guids=chart_guids)

        dashboard_queries = [v for k, v in dashboard_charts.get('referredEntities', dict()).items()
                             if v.get('typeName') == DashboardTypes.query and v.status == Status.ACTIVE]

        return self._serialize_dashboard_queries(dashboard_queries)

    def _get_tables(self, table_guids: List[str]) -> List[PopularTable]:
        """
        Return list of Tables.

        :param table_guids: list of table guids
        :return : List of Amundsen PopularTable objects.
        """
        table_entities = self.client.entity.get_entities_by_guids(guids=table_guids).entities
    ...
    def _get_dashboard_details(self, entity: AtlasEntityWithExtInfo) -> Dict:
        try:
            ...
            queries = self._get_dashboard_queries([q.get('guid') for q in
                                                   self._filter_active(relationships.get('charts', []))])
            query_names = [q.name for q in queries]

            tables = self._get_tables([t.get('guid')
    ...
    def _serialize_dashboard_summaries(self, guids) -> List[DashboardSummary]:
        """
    ...
    def get_resources_using_table(self, *,
        ...
        dashboard_guids = [d.get('guid')
                           for d in self._filter_active(table.entity[CommonKeys.rels].get('dashboards', []))]

        result = self._serialize_dashboard_summaries(dashboard_guids)
